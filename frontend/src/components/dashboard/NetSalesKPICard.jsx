...
/**
 * NetSalesKPICard Component
 * Displays Total Net Sales KPI with loading state
 */
export const NetSalesKPICard = ({ totalNetSales, loading, error, onClick }) => {
    ...
    >
      ...
      <div className="flex items-start justify-between mb-2 sm:mb-3">
        <div className="flex items-center gap-2 sm:gap-3">
          <div className="w-8 h-8 sm:w-10 sm:h-10 lg:w-11 lg:h-11 rounded-lg sm:rounded-xl flex items-center justify-center transition-transform group-hover:scale-110 bg-metric-green/10 text-metric-green">
        ...
        {loading ? (
          ...
          <>
            <h3 className="text-2xl sm:text-3xl font-bold text-foreground tracking-tight leading-tight">
              {formatCurrency(totalNetSales || 0)}
            </h3>
            <p className="text-[10px] sm:text-xs text-muted-foreground mt-1 sm:mt-1.5">
              Fuel Sales + Other Income
            </p>
          </>
        )}
      </div>
