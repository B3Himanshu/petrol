...
export const AvgPPLKPICard = ({ avgPPL, loading, error }) => {
  const formatPPL = (ppl) => {
    if (!ppl && ppl !== 0) return "0 p";
    return `${ppl.toFixed(2)} p`;
  };
    ...
    >
      <div className="flex items-start justify-between mb-2 sm:mb-3">
        <div className="flex items-center gap-2 sm:gap-3">
          <div className="w-8 h-8 sm:w-10 sm:h-10 lg:w-11 lg:h-11 rounded-lg sm:rounded-xl flex items-center justify-center transition-transform group-hover:scale-110 bg-metric-purple/10 text-metric-purple">
